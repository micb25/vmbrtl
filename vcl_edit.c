...
LRESULT CALLBACK VCL_EditProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
void VCL_EditUpdateFont(VCLEditProperties *Prop, HWND hwnd);
int iVCLEditCount = 0;
...
// -----------------------------------------------------------------------------
//  VCL_EditCreate(char *) - erzeugt eine Edit Komponente
// -----------------------------------------------------------------------------
HWND VCL_EditCreate(char *Caption)
{
 HWND hWnd;
 hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", NULL, /*WS_BORDER | WS_TABSTOP | ES_AUTOHSCROLL | ES_LEFT | WS_VISIBLE | WS_CHILD*/ WS_TABSTOP | WS_VISIBLE | WS_CHILD | ES_AUTOVSCROLL | ES_AUTOHSCROLL, CW_USEDEFAULT, CW_USEDEFAULT, 120, 21, LastCreatedWindow, NULL, RTL_HINSTANCE, NULL);  
 if (hWnd != NULL)
 {
   OldEditProc = (WNDPROC*)GetWindowLong(hWnd, GWL_WNDPROC);
   SetWindowLong(hWnd, GWL_WNDPROC, (LONG)&VCL_EditProc);          
...
// -----------------------------------------------------------------------------
//  VCL_EditMethod(hwnd, int) - ruft eine Methode auf
// -----------------------------------------------------------------------------
void VCL_EditMethod(VMBVar *VCLCom, unsigned int Method)
{
     char *P1, *P2, *P3, *P4, *P5;
...
// -----------------------------------------------------------------------------
//  VCL_EditMember(hwnd, int) - ändert einen Member 
// -----------------------------------------------------------------------------
void VCL_EditMember(VMBVar *VCLCom, unsigned int Method)
{
     char *szBuffer1 = NULL;
     ...
     szBuffer1 = (char *)VMBParseOneString();
        switch(Method) 
        {
                       ...
                       case 11:                    
                            ...
                            VCL_EditUpdateFont(Prop, VCLCom->hWnd);
                            break;                            
                       case 12:
                       ...
                       case 19:
                            cChar = *szBuffer1;
                            SendMessage(VCLCom->hWnd, EM_SETPASSWORDCHAR, (WPARAM) (UINT) cChar, 0);
                            RedrawWindow(VCLCom->hWnd, NULL, 0, RDW_ERASE | RDW_ALLCHILDREN | RDW_FRAME | RDW_INVALIDATE );
                            break;                               
                       default:
#ifdef DEBUG                               
...
// -----------------------------------------------------------------------------
//  VCL_EditGetMember(hwnd, uint) - liest einen Member
// -----------------------------------------------------------------------------
void VCL_EditGetMember(VMBVar *VCLCom, unsigned int Method)
{
     ...
     VCLEditProperties *Prop = (VCLEditProperties *)VCLCom->Properties;
        switch(Method) 
        {
                       ...
                       case 18:
                            ...
                            break;
                       case 19:
                            cChar = SendMessage(VCLCom->hWnd, EM_GETPASSWORDCHAR, 0, 0);
                            if (cChar == 0)
                            {
                             sprintf(szBuffer, "");          
                            } else {
                             sprintf(szBuffer, "%c", cChar);        
                            }
                            break;
                       default: 
#ifdef DEBUG                                
...
// -----------------------------------------------------------------------------
// VCL_EditProc(int, int, int, int) - Callback für Edits
// -----------------------------------------------------------------------------
LRESULT CALLBACK VCL_EditProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{    
     VMBVar *VCLCom;
     ...
     switch(uMsg) {
         case WM_RBUTTONUP:
             ...
             VMBDebug("Edit: WM_RBUTTONUP received!");
#endif                
         ...
         case WM_LBUTTONUP:
#ifdef SHOWWINDOWMESSAGES
             VMBDebug("Edit: WM_LBUTTONUP received!");
#endif                
              VCLCom = (VMBVar*)VMBFindVarByHWND(hwnd);
              if ( VCLCom )
              {
...
// -----------------------------------------------------------------------------
// VCL_EditUpdateFont(PROP, HWND) - Verändert den Font des Edits
// -----------------------------------------------------------------------------
void VCL_EditUpdateFont(VCLEditProperties *Prop, HWND hwnd)
{
