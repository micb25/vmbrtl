...
#ifndef VCL_GROUPBOX_C
#define VCL_GROUPBOX_C
...
LRESULT CALLBACK VCL_GroupBoxProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
void VCL_GroupBoxUpdateFont(VCLGroupBoxProperties *Prop, HWND hwnd);
int iVCLGroupBoxCount = 0;
...
// -----------------------------------------------------------------------------
//  VCL_GroupBoxCreate(char *) - erzeugt eine GroupBox Komponente
// -----------------------------------------------------------------------------
HWND VCL_GroupBoxCreate(char *Caption)
{
 HWND hWnd;
 hWnd = CreateWindowEx(0, "BUTTON", NULL, BS_GROUPBOX | WS_GROUP | WS_VISIBLE | WS_CHILD,  CW_USEDEFAULT, CW_USEDEFAULT, 120, 25, LastCreatedWindow, NULL, RTL_HINSTANCE, NULL);
 if (hWnd != NULL)
 {
   OldGroupBoxProc = (WNDPROC*)GetWindowLong(hWnd, GWL_WNDPROC);
   SetWindowLong(hWnd, GWL_WNDPROC, (LONG)&VCL_GroupBoxProc);          
...
// -----------------------------------------------------------------------------
//  VCL_GroupBoxMethod(hwnd, int) - ruft eine Methode auf
// -----------------------------------------------------------------------------
void VCL_GroupBoxMethod(VMBVar *VCLCom, unsigned int Method)
{
     char *P1, *P2, *P3, *P4, *P5;
     VCLGroupBoxProperties *Prop = (VCLGroupBoxProperties*)VCLCom->Properties;
     switch(Method) 
        {
                       case 1:
...
// -----------------------------------------------------------------------------
//  VCL_GroupBoxMember(hwnd, int) - ändert einen Member 
// -----------------------------------------------------------------------------
void VCL_GroupBoxMember(VMBVar *VCLCom, unsigned int Method)
{
     char *szBuffer1 = NULL;
     ...
     szBuffer1 = (char *)VMBParseOneString();
        switch(Method) 
        {
                       ...
                       case 10:                    
                            ...
                            VCL_GroupBoxUpdateFont(Prop, VCLCom->hWnd);
                            break;                            
                       case 11:
                            iBuffer = atoi(szBuffer1);
...
// -----------------------------------------------------------------------------
//  VCL_GroupBoxGetMember(hwnd, uint) - liest einen Member
// -----------------------------------------------------------------------------
void VCL_GroupBoxGetMember(VMBVar *VCLCom, unsigned int Method)
{
...
// -----------------------------------------------------------------------------
// VCL_GroupBoxProc(int, int, int, int) - Callback für GroupBox
// -----------------------------------------------------------------------------
LRESULT CALLBACK VCL_GroupBoxProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{    
     VMBVar *VCLCom;
     ...
     switch ( uMsg )
     {
            case WM_LBUTTONUP:
             ...
             VMBDebug("GroupBox: WM_LBUTTONUP received!");
#endif                
     ...
     return  CallWindowProc( (WNDPROC)OldGroupBoxProc, hwnd, uMsg, wParam, lParam);        
}

// -----------------------------------------------------------------------------
// VCL_GroupBoxUpdateFont(PROP, HWND) - Verändert den Font der GroupBox
// -----------------------------------------------------------------------------
void VCL_GroupBoxUpdateFont(VCLGroupBoxProperties *Prop, HWND hwnd)
{
